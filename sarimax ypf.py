...
# %% [Sección 2: DATOS]
empresa = 'YPF'
archivo = "K:/Rosental/Comun/Bolsa/Mesa Operaciones/Agustin/datos - ypf.xlsx"
df = pd.read_excel(archivo, sheet_name= "BD YPF")
df['fecha'] = pd.to_datetime(df['fecha'], format='%Y-%m-%d')
df = df.set_index('fecha')
df = df.sort_values(by='fecha')
fechas_post_eecc = df['fecha_post_eecc']
#Elimino columnas que no tengo todos los datos
df = df.drop(columns=["ROA_TO_ROE","BS_TOT_LIAB2","EV_TO_MARKET_CAP","BS_TOT_LIAB2"])

#saco variables que estan muy correlacionadas
df = df.drop(columns=["EBIT_TO_NET_SALES", "EARN_FOR_COM_TO_TOT_REV", "EARN_FOR_COMMON"])

variables_contables = ["EBITDA_TO_REVENUE", "CF_NET_INC", "IS_COMP_NET_INCOME" , "SALES_REV_TURN", "IS_OPER_INC" ,  "CF_FREE_CASH_FLOW","IS_COMP_SALES", "IS_COMPARABLE_EBITDA", "IS_COMPARABLE_EBIT"]
datos_eecc = df[variables_contables]
otras_variables = ["YIELD GOVT 10 PRE BALANCE", "SPY_pre_eecc" , empresa+"_pre_eecc"]

# USAR PRECIOS DE YFINANCE Y NO DE BLOOMBERG
start_date = df['fecha_pre_eecc'].min()
end_date =   df['fecha_pre_eecc'].max() + BDay(2)
...
#df['YPF_i_SPY'] = df[empresa+"_pre_eecc"] * df["SPY_pre_eecc"]
#otras_variables.append('YPF_i_SPY')
df['YIELD_i_YPF'] = df["YIELD GOVT 10 PRE BALANCE"] * df[empresa+"_pre_eecc"]
otras_variables.append('YIELD_i_YPF')
...
datos_train['YPF_pre_eecc'].plot(ax=ax, marker='o', linestyle='-', label='train')
datos_test['YPF_post_eecc'].plot(ax=ax, marker='o', linestyle='-', label='test')
...
adfuller_result = adfuller(df[empresa+"_post_eecc"])
...
kpss_result = kpss(df[empresa+"_post_eecc"])
print(f'KPSS Statistic: {kpss_result[0]}, p-value: {kpss_result[1]}')

datos_diff_1 = df[empresa+"_post_eecc"].diff().dropna()
...
plt.figure(figsize=(10, 6))
plt.plot(df.index, df[empresa+"_post_eecc"], marker='o', linestyle='-')
...
plot_acf(df[empresa+"_post_eecc"], ax=axs[0], lags=30, alpha=0.05)
...
fig, axs = plt.subplots(nrows=2, ncols=1, figsize=(7, 7), sharex=True)
plot_pacf(df[empresa+"_post_eecc"], ax=axs[0], lags=20, alpha=0.05)
...
# ==============================================================================
datos_diff_1_4 = df[empresa+"_post_eecc"].diff().diff(4).dropna()
...
#Cantidad de modelos analizados: 40.000. Lo corri a las 11:45 del 23/9/24. Terminó: 14:45
...
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 1, 2), seasonal_order=(2, 0, 1, 4)) # rsme 0.61
model = SARIMAX(endogena_train, exog=exogenas_train, order=(3, 1, 1), seasonal_order=(3, 1, 2, 4)) # rsme 0.88
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 1, 1, 4)) # rsme 2.5
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 3, 4)) # rsme 0.65
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 0.69
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 0, 1, 4)) # rsme 0.6
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 1, 4)) # rsme 0.65
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 1, 1), seasonal_order=(2, 0, 1, 4)) # rsme 0.62

model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(1, 2, 0, 4)) # rsme 4200
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 410
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 2, 2), seasonal_order=(2, 0, 1, 4)) # rsme 1.24
model = SARIMAX(endogena_train, exog=exogenas_train, order=(3, 2, 1), seasonal_order=(3, 1, 2, 4)) # rsme 1.9
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 1, 1, 4)) # rsme 26
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 2, 1), seasonal_order=(2, 0, 1, 4)) # rsme 308
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 3, 4)) # rsme 329
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 1, 4)) # rsme 432
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 2), seasonal_order=(2, 1, 1, 4)) # rsme 153
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 2), seasonal_order=(2, 0, 1, 4)) # rsme 1.22

#modelo 3. SARIMAX con outliers y CP
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 2, 2, 4)) # rsme 0.94
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 1, 2, 4)) # rsme 0.88
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 0.53
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 0, 2, 4)) # rsme 0.52
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 0.49
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(1, 0, 2, 4)) # rsme 0.98
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 0.53

model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 2, 2, 4)) # rsme 27
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 1, 2, 4)) # rsme 1.8
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 11.4
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 2), seasonal_order=(2, 0, 2, 4)) # rsme 1.1
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 5.8
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(1, 0, 2, 4)) # rsme 3.18
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 11.45

#modelo 4. SARIMAX con outliers , CP y YIELD_i_SPY
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 3.23
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 1.46
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 1, 2, 4)) # rsme 12.7
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 1, 4)) # rsme 10.6
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 1.46
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 3), seasonal_order=(2, 0, 2, 4)) # rsme  1.84
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(1, 0, 2, 4)) # rsme 4
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 1, 2, 4)) # rsme 12
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(1, 0, 2, 4)) # rsme 4
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 0, 1, 4)) # rsme 7
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 0, 2, 4)) # rsme 1.34

model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 137
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 2), seasonal_order=(2, 0, 2, 4)) # rsme 34
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 35
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 137
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 1, 2, 4)) # rsme 337
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 1, 4)) # rsme 87

#modelo 4. SARIMAX con outliers , CP y YPF_i_SPY
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 0.6
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 1.1
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 1, 2, 4)) # rsme 0.96
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 1, 4)) # rsme 1.47
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 1.1
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 3), seasonal_order=(2, 0, 2, 4)) # rsme  0.62
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(1, 0, 2, 4)) # rsme 1
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 1, 2, 4)) # rsme 0.86
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(1, 0, 2, 4)) # rsme 1
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 0, 1, 4)) # rsme 1.6
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 0, 2, 4)) # rsme 0.51

model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 320
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 2), seasonal_order=(2, 0, 2, 4)) # rsme 2.9
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 83
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 320
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 1, 2, 4)) # rsme 27
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 1, 4)) # rsme 305

#modelo 4. SARIMAX con outliers , CP y YPF_i_SPY
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 0.54
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 0.53
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 1, 2, 4)) # rsme 0.58
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 1, 4)) # rsme 0.68
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 1), seasonal_order=(2, 0, 2, 4)) # rsme 0.53
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 3), seasonal_order=(2, 0, 2, 4)) # rsme 0.503 
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(1, 0, 2, 4)) # rsme 0.47
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 1, 2, 4)) # rsme 0.54
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 0, 1, 4)) # rsme 0.59
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(2, 0, 2, 4)) # rsme 0.49
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 1, 2), seasonal_order=(1, 0, 2, 4)) # rsme 0.47

model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 812
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 2), seasonal_order=(2, 0, 2, 4)) # rsme 0.79
model = SARIMAX(endogena_train, exog=exogenas_train, order=(2, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 15
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 2, 4)) # rsme 812
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 1, 2, 4)) # rsme 1.1
model = SARIMAX(endogena_train, exog=exogenas_train, order=(1, 2, 1), seasonal_order=(2, 0, 1, 4)) # rsme 99
...
plt.title('Residuos del modelo 4')

plt.hist(residuos, bins=20, edgecolor='black')
plt.title('Histograma de los residuos del modelo 4')

stats.probplot(residuos, dist="norm", plot=plt)
plt.title('Gráfico Q-Q de los residuos del modelo 4')
...
plt.xlabel('Fecha')
plt.ylabel('Precio')
plt.title('Predicciones del modelo 4')
plt.legend()
...
mpf.plot(df_velas, type='candle', style='charles',addplot=add_plot, title='Gráfico de Velas de YPF con la prediccion realizada', ylabel='Precio')

'''
# con info de bloomberg
df_velas = pd.read_excel(archivo, sheet_name= "YPF-VELAS")
...
# Con el mejor modelo SARIMAX: (1, 1, 2)(2, 0, 1, 4).
...
variables_seleccionadas, mejor_modelo = forward_selection_sarimax(df, 'YPF_post_eecc', variables_contables, p, d, q, P, D, Q, s)
...
# Llamar a la función con el DataFrame 'df', la variable target 'YPF_post_eecc' y las variables contables
variables_seleccionadas, mejor_modelo = backward_selection_sarimax(df, empresa+'_post_eecc', variables_contables, 1, 1, 2, 2, 0, 1, 4)
